...
class Controler {
	...
	function askSignIn ( id:String, pwd:String) {
		...
		server.ask( { req:"signIn", data:Json.stringify({id:id, pwd:pwd})  } );
	...
	function onAnswerSignIn (e:StandardEvent) {
		...
		if (answ == "error") { 
			...
			else if (msg == "loginFrozen") {
				var d = g.numVal(e.result.delay, 0);
				var mn = Math.floor(d / 60); var sec = d % 60; var id = e.result.id;
				var arr:Array<String> = [];
				arr.push(lang.loginFrozen1 + Std.string(mn));
				arr.push(lang.loginFrozen2 + Std.string(sec));
				arr=arr.concat(lang.loginFrozen3);
				g.alert(arr,onErrorCallBackSignIn);					
			}
			else if (msg!="") {		
				g.alert(lang.serverReadError+msg,onErrorCallBackSignIn);
			}
			else {		
	...
	function askSignUp ( id:String, pwd:String) {
		...
		model.currUserPwd = pwd;
		server.ask( { req:"signUp", data:Json.stringify({id:id, pwd:pwd})  } );
	...
	function goMain () {
		...
		if (model.dataIsEmpty()) {
			g.alert(lang.alertBeforeFolderCreation,null,lang.startAlertTitle);
		}
		else {			
	...
	function setupCurrentFormOrFolder (f:AbstractFolderFormField) {	
		// deselect previous
	...
	function setStateOfAddButtons (fo:AbstractFolderFormField) {
		if (model.mode=="admin") {
			if (fo.is("Folder")) {
